...
notify-send "HorneroConfig" "Applying Rose Pine rice... 🌹"
...
if [[ ${#image_files[@]} -eq 0 ]]; then
  notify-send "HorneroConfig" "Rose Pine: no wallpapers found in backgrounds/."
  exit 0
fi
...
ANIMATIONS_FILE="$HOME/.config/hypr/hyprland.conf.d/animations-standard.conf"
if [[ -f $ANIMATIONS_FILE ]]; then
  ln -sf "$ANIMATIONS_FILE" "$HOME/.config/hypr/hyprland.conf.d/animations-current.conf"
fi

notify-send "HorneroConfig" "Rose Pine rice applied! 🌹 Elegant and serene."
