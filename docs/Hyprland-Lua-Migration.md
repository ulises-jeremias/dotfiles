...
- `Rice.qml` writes `hyprland.lua.d/animations-current.lua` as a tiny
  `require("hyprland.lua.d/animations-<profile>")` wrapper.
- `hyprland.lua` loads `animations-current.lua` when present and falls back to
  `animations.lua` otherwise.
...
Previously parsed `hyprland.conf.d/keybindings.conf` directly. After the Lua
migration, the runtime source of truth is `hyprctl binds -j`:

- `dots-keyboard-help` reads `hyprctl binds -j` when Hyprland is running.
- If Lua is not active and legacy `hyprland.conf.d/keybindings.conf` still
  exists outside this migration, the script can fall back to parsing it.
- Category filters infer groups from descriptions when available, then from
  dispatcher/argument patterns.
...
```lua
hl.bind("mod + key", hl.dsp.whatever(...))
```
